...
export interface AnalyticsConfig {
  ...
  fetch?: typeof fetch;
  /**
   * Root domain(s) allowed as cross-domain tracking targets. Must match the
   * workspace's Additional Domains configuration in the dashboard. Links to
   * these hostnames are rewritten with visitor/session params instead of
   * being tracked as exit links.
   */
...
export interface AutoCapturePageviewsOptions {
  enabled?: boolean;
...
export interface ConvrsWebConfig {
  /** Your website/workspace ID */
  websiteId: string;
  /** Root domain this script is installed on, e.g. "example.com" */
  domain: string;
  /** Override the ingestion endpoint */
  apiUrl?: string;
  /** Enable debug logging */
  debug?: boolean;
  /** Interval in ms between auto-flushes */
  flushInterval?: number;
  /** Max events to batch before auto-flush */
  maxQueueSize?: number;
  /** Skip persistent visitor identity, use ephemeral/server-assigned IDs */
  cookieless?: boolean;
  /** Called when a cookieless visitor ID is assigned/cleared */
  onCookielessVisitorId?: (visitorId: string) => void;
  /** Allow tracking inside an iframe (disabled by default) */
  allowIframe?: boolean;
  /** Allow tracking on localhost/file:// (disabled by default) */
  allowLocalhost?: boolean;
  /** Automatically fire pageviews on SPA route changes */
  autoCapturePageviews?: boolean | AutoCapturePageviewsOptions;
  /**
   * Other root domains this visitor's identity should carry across, e.g.
   * ["app.io", "shop.example.com"]. Must match the workspace's Additional
   * Domains setting in the dashboard — this is not synced automatically.
   */
  allowedHostnames?: string[];
  osVersion?: string;
  appVersion?: string;
