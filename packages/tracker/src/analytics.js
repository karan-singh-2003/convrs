 ...
 *   data-allow-localhost="true"                    Enable tracking on localhost
 *   data-allow-file-protocol="true"               Enable tracking on file://
 *   data-allowed-hostnames="app.io,shop.io"        Cross-domain tracking targets
 *
 * ── Pre-init queue (place before script tag) ──────────────────────────────────
 *   window.analytics = window.analytics || { q: [] };
 *   window.analytics.q.push(["trackEvent", "signup", { plan: "pro" }]);
 ...
 *   window.analytics.trackPageview()
 *   window.analytics.identify({ user_id: "u_123", name: "Jane" })
 *   window.analytics.buildCrossDomainUrl("https://app.io/signup")
 *   window.analytics.getTrackingParams()
 *
 * ── Declarative HTML API ──────────────────────────────────────────────────────
 *   <button data-goal="signup" data-goal-plan="pro">Sign up</button>
 *   <section data-scroll-goal="section_viewed" data-scroll-threshold="0.5"></section>
...
!(function () {
  ...
  var _allowedHostnames = (attr("data-allowed-hostnames") || "")
    .split(",")
    .map(function (h) {
  ...
  var CROSS_DOMAIN_VID_PARAM = "_cv_vid";
  var CROSS_DOMAIN_SID_PARAM = "_cv_sid";
  ...
  var SESSION_KEY = "_cv_sid";
  var VISITOR_KEY = "_cv_vid";
  var SESSION_SECS = 30 * 60;
  var VISITOR_SECS = 365 * 24 * 3600;
  var ENTRY_KEY = "_cv_entry";
  ...
  function setCookie(name, value, maxAgeSec) {
    ...
    if (!isLocalhost(window.location.hostname)) {
      ...
      var isMainDomainOrSubdomain =
        _domain &&
        (currentHostname === _domain || currentHostname.endsWith("." + _domain));

      if (isMainDomainOrSubdomain) {
        cookie += "; domain=." + _domain.replace(/^\./, "");
      } else {
        cookie += "; domain=." + currentHostname.replace(/^\./, "");
      }
    }
  ...
  function getSessionId() {
    var sid = getCookie(SESSION_KEY);
    if (!sid) sid = uuid();
    setCookie(SESSION_KEY, sid, SESSION_SECS);
    return sid;
  }
  ...
  // ─── CROSS-DOMAIN TRACKING: restore session from URL params ────────────────
  // Runs once on init, before any cookie is read/generated, so a visitor
  // arriving from an allowed domain keeps the same visitor/session identity
  // instead of getting a fresh one on the new domain.
  function restoreCrossDomainSession() {
    try {
      var params = new URLSearchParams(window.location.search);
      var vid = params.get(CROSS_DOMAIN_VID_PARAM);
      var sid = params.get(CROSS_DOMAIN_SID_PARAM);
      if (!vid && !sid) return;
      ...
      log("info", "Restored cross-domain session", { vid: vid, sid: sid });

      // Strip the tracking params from the visible URL without adding a
      // history entry or triggering a duplicate pageview (pathname is
      // unchanged, so the pushState/replaceState patch below won't refire).
      params.delete(CROSS_DOMAIN_VID_PARAM);
      params.delete(CROSS_DOMAIN_SID_PARAM);
      ...
      log("warn", "Failed to restore cross-domain session");
    }
  }
  ...
  // ─── CROSS-DOMAIN TRACKING: hostname allowlist check ────────────────────────
  // Distinct from isInternalHost — this only answers "is this an explicitly
  // allowed *different* domain", used for link rewriting / buildCrossDomainUrl.
  function isAllowedCrossDomainHost(hostname) {
    if (!hostname) return false;
    var h = hostname.toLowerCase();
    var current = window.location.hostname.toLowerCase();
    if (h === current) return false;
    for (var i = 0; i < _allowedHostnames.length; i++) {
      if (h === _allowedHostnames[i]) return true;
    }
    return false;
  }
  ...
  function isInternalHost(hostname) {
    ...
    return isAllowedCrossDomainHost(h);
  }
  ...
  // ─── CROSS-DOMAIN TRACKING: URL builder + params helper ─────────────────────
  function buildCrossDomainUrl(url) {
    var parsed;
    try {
      parsed = new URL(url, window.location.href);
    } catch (_) {
      log("warn", "buildCrossDomainUrl: invalid URL", url);
      return url;
    }
    if (!isAllowedCrossDomainHost(parsed.hostname)) {
      log(
        "warn",
        "buildCrossDomainUrl: hostname not in data-allowed-hostnames, returning unchanged —",
        parsed.hostname
      );
      return parsed.href;
    }
    parsed.searchParams.set(CROSS_DOMAIN_VID_PARAM, getVisitorId());
    parsed.searchParams.set(CROSS_DOMAIN_SID_PARAM, getSessionId());
    return parsed.href;
  }

  function getTrackingParams() {
    var params = {};
    params[CROSS_DOMAIN_VID_PARAM] = getVisitorId();
    params[CROSS_DOMAIN_SID_PARAM] = getSessionId();
    return params;
  }
  ...
  function onActivation(e) {
    ...
    // CROSS-DOMAIN TRACKING: rewrite the href in place, before the browser's
    // default navigation runs (this listener is capture-phase), so the
    // visitor/session identity carries over. Applies to plain <a> tags only —
    // JS-driven navigation should use buildCrossDomainUrl()/getTrackingParams().
    try {
      var resolved = new URL(href, window.location.href);
      if (isAllowedCrossDomainHost(resolved.hostname)) {
        var rewritten = buildCrossDomainUrl(resolved.href);
  ...
  // ─── BOOTSTRAP ─────────────────────────────────────────────────────────────
  function init() {
    restoreCrossDomainSession(); // must run before any cookie/session is read
    initScrollTracking();
    trackPageview();
    drainQueue();
    startHeartbeat();
  ...
  window.analytics = {
    ...
    trackEvent: trackEvent,
    identify: identify,
    buildCrossDomainUrl: buildCrossDomainUrl,
  ...
  // CROSS-DOMAIN TRACKING: attach helpers to the callable convrs function too,
  // since window.convrs is a function, not a plain object.
  convrs.buildCrossDomainUrl = buildCrossDomainUrl;
  convrs.getTrackingParams = getTrackingParams;
