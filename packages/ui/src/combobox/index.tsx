...
function isMultipleSelection(multiple: boolean | undefined): multiple is true {
...
export function Combobox({
  ...
  const isMultiple = isMultipleSelection(multiple);
  const setMultipleSelected = setSelected as
    | ((options: ComboboxOption[]) => void)
    | undefined;
  const setSingleSelected = setSelected as
    | ((option: ComboboxOption | null) => void)
    | undefined;
  ...
  return (
    <Popover
      ...
      popoverContentClassName={cn(
        "w-[var(--radix-popover-trigger-width)] rounded-xl",
      ...
      content={
        ...
        >
          <Command loop shouldFilter={shouldFilter}>
            {!hideSearch && (
              <div className=" border-b border-neutral-200">
                <Command.Input
                  ...
                  className={cn(
                    "text-content-emphasis w-full text-sm font-display placeholder:text-content-muted grow bg-transparent  py-2.5 pl-4 pr-2 outline-none shadow-none focus:ring-0 focus:border-none focus:outline-none",
                    inputClassName
                  )}
                  onKeyDown={(e) => {
                    if (
            ...
            <ScrollContainer
              className={cn(
                "max-h-[min(50vh,250px)] w-full",
                onCreate && !multiple && "max-h-[calc(min(50vh,250px)-3.5rem)]"
              )}
            >
              <Command.List
                className={cn("flex w-full min-w-0 flex-col gap-1 p-1")}
              >
  ...
  return (
    <>
      <DisabledTooltip disabledTooltip={option.disabledTooltip}>
        <Command.Item
          className={cn(
            "flex cursor-pointer font-display items-center text-neutral-600 gap-3 rounded-md px-3 py-2 text-left text-sm",
