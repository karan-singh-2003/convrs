...
//   console.log("params",start,end,interval)
...
import { AnalyticsContext } from "./analytics-providers";
import { formatDuration } from "./analytics-tabs";
import { useSocialActivityModal } from "../modals/use-social-activity-modal";
...
// Mirrors AnalyticsProvider's previousQueryUrl fallback mapping — start/end
// from AnalyticsContext are only populated for a literal custom date range;
// preset intervals (24h/7d/30d/...) only carry `interval` as a string, with
// start/end left undefined. Without this, the activity-timeline fetch below
// would never fire for the default preset views, only custom ranges.
...
function resolveDateRange(
  ...
  const resolvedStart = subDays(resolvedEnd, days);
                    ...
                    <div className="space-y-0">
                      ...
                      <div className="space-y-0">
                        {previewItems.map((item, i) => (
                          <div
                            key={`${item.handle}-${i}`}
                            className="flex items-start gap-2 rounded-lg bg-bg-subtle/50 "
                          >
                            <img
                              src={item.avatarUrl ?? undefined}
                              alt={item.handle}
