...
import { groupReferrersByDisplayName } from "@/lib/swr/use-channel-breakdown";
...
export function useAnalyticsFilters({
  ...
  // --- Referrer grouping helpers -------------------------------------
  // Maps a display-name group (e.g. "X") back to its underlying raw
  // domains (t.co, twitter.com, ...), built from the same `referers`
  // data used to populate the dropdown. Used to expand a group label
  // into raw domains whenever the filter is applied/removed, since the
  // backend only ever matches on raw referer values.
  // (declared after `referers` is fetched below; see refererDomainsByLabel)
  ...
  const activeFilters = useMemo(() => {
    ...
    VALID_ANALYTICS_FILTERS.forEach((filter) => {
      ...
      if (value) {
        const parsed = parseFilterParam(value);
        if (parsed) {
          // Referrer values are raw domains in the URL (t.co, twitter.com),
          // but the chip should read as the grouped display name ("X").
          // Dedupe so two raw domains under the same group render as one
          // chip value instead of "X, X".
          const displayValues =
            filter === "referer"
              ? Array.from(
                  new Set(parsed.values.map((v) => getReferrerDisplayName(v)))
                )
              : parsed.values;
  ...
  // Raw domains grouped by display name, e.g. { X: ["t.co","twitter.com"], Google: ["google.com"] }.
  // Used to expand a group label ("X") back into raw domains for onSelect/onRemove.
  const refererDomainsByLabel = useMemo(() => {
    const map: Record<string, string[]> = {};
    (referers ?? []).forEach((r) => {
      const label = getReferrerDisplayName(r.referer);
      map[label] = [...(map[label] ?? []), r.referer];
    });
    return map;
  }, [referers]);

  const expandRefererValue = useCallback(
    (value: string) => refererDomainsByLabel[value] ?? [value],
    [refererDomainsByLabel]
  );

  // Grouped referer options for the dropdown — one row per display name
  // ("X"), not one per raw domain, matching what the chip now shows.
  const refererGroupOptions = useMemo(() => {
    if (!referers) return null;
    const grouped = groupReferrersByDisplayName(
      referers.map((r) => ({
        referer: r.referer,
        count: r.count ?? 0,
        revenue: r.revenue ?? 0,
    ...
    return grouped.map((g) => ({
      value: g.label,
      label: g.label,
      right: nFormatter(g.value, { full: true }),
    }));
  }, [referers]);
  ...
  const filters: ComponentProps<typeof Filter.Select>["filters"] = useMemo(
    () => [
      ...
      ...(programPage ? [] : []),
      {
        key: "referer",
        label: "Referrer",
        labelPlural: "referrers",
        // options are now grouped display names ("X"), not raw domains
        options: refererGroupOptions,
      },
      {
        key: "hostname",
        label: "Hostname",
  ...
  const onSelect = useCallback(
    async (key, value) => {
      // Referrer selections are group labels ("X") — expand into raw
      // domains before writing to the query string, since the backend
      // only ever matches on raw referer values.
      if (key === "referer") {
        const labels = Array.isArray(value) ? value : [value];
        const rawDomains = Array.from(
          new Set(labels.flatMap(expandRefererValue))
        );
        ...
        const currentParam = searchParamsObj[key];
        const isNegated = currentParam?.startsWith("-") ?? false;
        const existing = currentParam
          ? parseFilterParam(currentParam)?.values ?? []
          : [];
        const merged = Array.from(new Set([...existing, ...rawDomains]));

        queryParams({
          set: {
            [key]: isNegated ? `-${merged.join(",")}` : merged.join(","),
          },
          del: "page",
          scroll: false,
        });
  ...
  const onRemove = useCallback(
    (key, value) => {
      ...
      // For referer, `value` is a group label ("X") — remove every raw
      // domain under that group, not just a literal "X" (which never
      // exists in the actual query string).
      const toRemove =
        key === "referer" ? expandRefererValue(value) : [value];

      const newValues = parsed.values.filter((v) => !toRemove.includes(v));
    ...
    [queryParams, searchParamsObj, parseFilterParam, expandRefererValue]
  );
