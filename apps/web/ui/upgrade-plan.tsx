...
import useWorkspace from "@/lib/swr/use-workspace";
import { Button } from "@repo/ui";
import { APP_DOMAIN, cn, getPlanDetails, isDowngradePlan, type PricingFamily } from "@repo/utils";
import { usePathname, useSearchParams } from "next/navigation";
import { useState } from "react";
import { toast } from "sonner";
...
export function UpgradePlanButton({
  ...
  const currentFamily: PricingFamily = (currentPlanFamily as PricingFamily) ?? "standard";

  const targetPlan = getPlanDetails({ plan, family }).plan;

  // ── Derive button state ───────────────────────────────────────────────────
  const isCurrentPlan =
    currentPlanName?.toLowerCase() === plan.toLowerCase() && currentFamily === family;
