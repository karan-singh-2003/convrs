...
export default function SocialIntegrationSettingsCard() {
  ...
  async function removeKeyword(id: string) {
    await fetch(`/api/workspaces/${slug}/social/keywords/${id}`, {
      method: "DELETE",
    });
    mutate(`/api/workspaces/${slug}/social/keywords`);
  }
  ...
  async function removeHandle(id: string) {
    await fetch(`/api/workspaces/${slug}/social/attribution-handles/${id}`, {
      method: "DELETE",
    });
    mutate(`/api/workspaces/${slug}/social/attribution-handles`);
  }
            ...
            <div className="flex items-center gap-4 justify-between">
              <div>
                ...
                <p className="mt-1 text-[13px] font-display text-content-subtle">
                  Connect your X account to monitor mentions, discover
                  conversations, and attribute traffic from social media.
                </p>
              </div>
  ...
  return (
    <AnimatedSizeContainer height>
      <div className="bg-bg-card rounded-2xl">
        ...
        <div className="p-2 px-7 space-y-3">
          {!isXConnected && (
            <div className="rounded-lg bg-bg-subtle px-2 py-2 text-[13px] font-display text-content-subtle">
              X isn't connected yet — keywords and link attribution accounts can
              still be added below, but tracking won't start until X is
              connected.
            </div>
          )}
          ...
          {activeTab === "Mentions" && (
            <>
              <p className="text-[13px] font-display text-content-subtle">
                Add brand names, product names, or campaign hashtags. Your
                domain is tracked automatically on both X and Reddit — no need
                to add it here. 4–15 characters, case insensitive.
              </p>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onKeyDown={(e) => e.key === "Enter" && addKeyword()}
                  placeholder="e.g. Convrs, @convrsdev, #convrs"
                  className="flex-1 h-10 rounded-lg border focus:ring-0 focus:outline-0 focus:border-border-default border-border-subtle bg-bg-default px-3 text-[13.5px] font-poppins text-content-default placeholder:text-content-subtle focus:outline-none"
                />
                <button
                  onClick={addKeyword}
                  className="h-10 px-4 rounded-lg bg-bg-inverted text-content-inverted text-[13px] font-poppins font-medium"
                >
                  Add
                </button>
              </div>
          ...
          {activeTab === "Link attribution" && (
            <>
              <p className="text-[13px] font-display text-content-subtle">
                Only add accounts that actually have your website link somewhere
                in their profile — your own account, or an affiliate account.
                Traffic from their profile page will be attributed directly to
                them.
              </p>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onKeyDown={(e) => e.key === "Enter" && addHandle()}
                  placeholder="e.g. @levelsio"
                  className="flex-1 h-10 rounded-lg border border-border-subtle bg-bg-default px-3 text-[13px] font-poppins text-content-default placeholder:text-content-subtle focus:outline-none"
                />
                <button
                  onClick={addHandle}
                  className="h-10 px-4 rounded-lg bg-bg-inverted text-content-inverted text-[13.5px] font-display font-medium"
                >
                  Add
                </button>
              </div>
              ...
              <div className="space-y-2">
                {handles.map((h) => (
                  ...
                  >
                    <span className="text-[13.5px] font-display text-content-default">
                      @{h.handle}{" "}
                      <span className="text-content-subtle uppercase text-[11px]">
                        {h.platform}
                      </span>
                    </span>
                    <button
