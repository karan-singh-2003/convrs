...
import { PageWidthWrapper } from "@/ui/layout/page-width-wrapper";
import SettingsChildrenLayout from "@/ui/workspaces/SettingsChildrentLayout";
import { AnimatedSizeContainer, Button, LoadingSpinner } from "@repo/ui";
import { useEffect, useState } from "react";
import { toast } from "sonner";
import {
  PROVIDERS,
  ...
  async function handleConnect() {
    ...
    try {
      const res = await fetch(`/api/integrations/${provider}/connect`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...buildConnectPayload(provider, form),
          workspaceId,
        }),
      });
      const data = await res.json();
    ...
    <PageWidthWrapper>
      <AnimatedSizeContainer height>
        ...
        >
          ...
          {error ? (
            <p className="mt-3 text-sm font-display text-red-500">{error}</p>
          ) : null}
        </SettingsChildrenLayout>
        <SettingsChildrenLayout title="" description="" className="mt-5">
          <div className="flex h-full flex-col gap-6 rounded-2xl bg-bg-card ">
            <div className="px-6 pt-4 flex flex-col gap-4">
              <div className="flex flex-col">
                <h1 className="font-display text-content-default text-base font-medium">
                  Currency
                </h1>
                <h1 className="font-display text-content-subtle text-[13.5px]">
                  Used for all revenue and payment conversions
                </h1>
              </div>
              <Currency
