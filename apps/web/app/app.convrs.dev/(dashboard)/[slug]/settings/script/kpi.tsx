...
export default function KPI() {
    ...
    const hasUnsavedChange =
        ...
        (kpi === "Goal"
            ...
            (kpiRes.data.kpiRevenueMetric ?? "revenue") !== revenueMetric);
    ...
    const handleSave = async () => {
        ...
        try {
            const res = await fetch(`/api/workspaces/${workspaceIdOrSlug}/kpi`, {
                ...
                body: JSON.stringify(
                    kpi === "Revenue"
                        ? { kpiType: "revenue", kpiRevenueMetric: revenueMetric }
            ...
            toast.success(
                kpi === "Revenue"
                    ? `KPI set to ${revenueMetric === "mrr" ? "MRR" : "Revenue"}`
                        ...
                        <div className="space-y-3">
                            ...
                            {kpi === "Revenue" && (
                                <RadioGroup
                                    value={revenueMetric}
                                    onValueChange={(v) =>
                                        setRevenueMetric(v === "mrr" ? "mrr" : "revenue")
                                    }
                                    className="space-y-2"
                                >
                                    ...
                                    <div className="flex items-center rounded-2xl bg-bg-card border border-border-subtle px-4 pb-2 gap-4 pt-[0.4rem]">
                                        <RadioGroupItem value="mrr" id="option-mrr" />
                                        <div>
                                            <Label
                                                htmlFor="option-mrr"
                                                className="font-display font-medium text-content-default"
                                            ...
                                            <h1 className="font-display text-[13.5px] font-medium text-content-subtle">
                                                Shows how your monthly recurring revenue changes over time
                                            </h1>
                                        </div>
                                    </div>
                                </RadioGroup>
