 ...
 *
 * Triggered by: subscription.active
 * Fires once: first payment succeeded (or, for a trial-backed subscription
 * created with subscription_data.trial_period_days, once checkout/card
 * capture succeeds — the trial clock starts, no charge yet).
 *
 * This file now only handles what's SPECIFIC to first activation:
 *   - resolving workspaceId from metadata
 *   - the dodoCustomerId uniqueness/conflict dance
 *   - sending the upgrade email + completing onboarding
 * Everything about resolving plan/family and writing the Workspace row is
 * delegated to applySubscriptionPlan() (lib/billing/apply-subscription-plan.ts),
 * shared with subscription-updated.ts.
 */
...
import type { WorkspaceProps } from "@/lib/types";
import type { DodoSubscriptionPayload } from "@/lib/dodo/types";
import { applySubscriptionPlan } from "@/lib/billing/apply-subscription";
...
export async function subscriptionActive(data: DodoSubscriptionPayload) {
  ...
  // ── Idempotency & conflict check on dodoCustomerId ────────────────────────
  // Case 1: this workspace already has a *different* dodoCustomerId assigned.
  if (
    existingWorkspace.dodoCustomerId &&
    existingWorkspace.dodoCustomerId !== data.customer.customer_id
  ...
  // Case 2: this workspace has no dodoCustomerId yet, but the incoming
  // customer_id might already belong to a DIFFERENT workspace. dodoCustomerId
  // is @unique on Workspace, so writing it here would violate that
  // constraint — check ahead of time to skip the field gracefully instead
  ...
  let result;
  try {
    result = await applySubscriptionPlan({
      workspaceId,
      data,
      extraData: shouldSetCustomerId
        ? { dodoCustomerId: data.customer.customer_id }
        : undefined,
    });
  } catch (err: any) {
    // Defensive fallback for the race window between the check above and
    // the write (e.g. two webhooks for the same customer arriving at once).
    if (err?.code === "P2002" && shouldSetCustomerId) {
      console.warn(
        `[dodo/subscription-active] dodoCustomerId race detected for workspace ${workspaceId}, retrying without it`
      );
      result = await applySubscriptionPlan({ workspaceId, data });
    } else {
      throw err;
    }
  }
  ...
  const users = result.workspace.users.map(({ user }) => ({
    id: user.id,
    email: user.email,
    name: user.name,
  }));
