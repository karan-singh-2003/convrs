 ...
 *   subscription.plan_changed – upgrade / downgrade via Change Plan API
 *
 * All plan/family resolution and the Workspace write now live in
 * applySubscriptionPlan() (shared with subscription-active.ts). This file
 * only does what's specific to "an existing subscription changed": find the
 * workspace by its already-known dodoSubscriptionId.
 */
...
export async function subscriptionUpdated(data: DodoSubscriptionPayload) {
  ...
  if (!workspace) {
    // Could be a brand-new subscription before subscription.active fired.
    // Safe to skip — subscription.active handles the first write.
    return NextResponse.json({ received: true });
  }

  await applySubscriptionPlan({ workspaceId: workspace.id, data });
